...
export async function listGitHubIssueComments(
  ...
  const response = await fetchGitHub(
    `https://api.github.com/repos/${repo}/issues/${issueNumber}/comments?${searchParams.toString()}`,
    {
      method: "GET",
      headers: getHeaders(token),
    },
  ...
  if (!response.ok) {
    throw new Error(`GitHub issue comments list failed: ${response.status} ${await response.text()}`);
  }
