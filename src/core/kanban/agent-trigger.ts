...
export function buildTaskPrompt(
  ...
  const availableTools = isBacklogPlanning
    ? [
        ...
        "- **confirm_feature_tree_story_context**: Confirm the strongest feature-tree match and get a normalized contextSearchSpec plus a prompt-ready `feature_tree` YAML block",
  ...
  const instructions = isBacklogPlanning
    ? [
        ...
        "4. Prefer feature-tree confirmation first: call `load_feature_tree_context` or `confirm_feature_tree_story_context` before broader Grep/Glob when feature-tree evidence exists.",
