...
const IRON_RULES: ConstitutionRule[] = [
  {
    ...
    severity: "iron",
    checkType: "regex",
    pattern: "(real|float|decimal|numeric)\\s*\\(\\s*['\"]\\w*(amount|price|balance|cost|fee|charge)\\w*['\"]",
    fileGlob: "schema.ts",
    negate: false,
    description: "Money fields must use integer type with Cents suffix. REAL/FLOAT/DECIMAL on money fields is forbidden.",
  },
  {
    id: "C3",
    title: "订单状态机",
  ...
  },
  {
    ...
    severity: "iron",
    checkType: "regex",
    pattern: "status\\s*[=:]\\s*['\"]?(active|running|inactive)['\"]?",
    fileGlob: "schema.ts",
    negate: false,
    description: "Merchant status must only be open/pause/closed. No status='active' or status='running' in schema.",
  },
  {
    id: "C6",
    title: "API路径规范",
