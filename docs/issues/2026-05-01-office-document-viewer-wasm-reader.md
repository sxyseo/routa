...
- [x] DOCX preserves Walnut-style structural empty paragraphs for blank documents and terminal block paragraphs, reducing real-world element/paragraph count drift.
- [x] DOCX mirrors Walnut scalar quirks for real-world invalid OOXML: uppercase RGB colors, integer-only paragraph spacing/run font size, and `contextualSpacing`-only empty paragraph fallback to docDefaults.
...
- [ ] DOCX full style inheritance depth: protocol-level resolution now covers docDefaults spacing, paragraph style summaries, renderer-side paragraph `basedOn`, direct run overrides, image/table bbox parity for key layout cases, and table bbox from actual table width. `docx_style_section_contract.docx` confirms Walnut does not materialize character-style `basedOn` into run `textStyle`; `docx_table_style_contract.docx` confirms Walnut does not materialize table-style conditional shading into cell fill. Remaining work is full paragraph style materialization, latent style defaults, false-value overrides, and richer table border/span/style regions.
...
- Walnut uses the first section's page setup as the root document page size for multi-section DOCX files.
...
- Walnut preserves a text element for structurally significant empty paragraphs, including blank documents and terminal block paragraphs. Routa now mirrors that behavior without applying inherited default spacing to a purely empty paragraph.
...
- 2026-05-02 Walnut/Routa protocol scan on the same 166-file corpus: Routa succeeded on 166/166; Walnut failed on 77/166 (`65` decimal integer attribute failures, `9` duplicate key failures, `3` invalid enum failures). Among the 89 files both readers decoded, full semantic parity improved from 17/89 to 48/89 after preserving Walnut-style empty paragraphs, normalizing RGB casing, using strict integer parsing for paragraph spacing/run font size, and honoring `contextualSpacing` on terminal empty paragraphs. Raw proto bytes still matched on 0/89.
- The same post-fix 89-file scan reduced `elementCountMatches` failures from 53 to 29, `elementTypeCountsMatch` from 53 to 29, `paragraphCountMatches` from 54 to 30, `paragraphSpacingSignaturesMatch` from 66 to 38, and `textRunStyleSignaturesMatch` from 45 to 18. The dominant remaining mismatches are paragraph spacing (`38`), paragraph/element counts (`30`/`29`), image digests (`22`), run style signatures (`18`), text run counts (`15`), section shapes/counts (`13`), table shapes/bbox (`9`), and page size (`9`).
- None of the previously full-matching files regressed; the newly full-matching files are mostly blank/near-empty, terminal-empty-paragraph, RGB casing, decimal spacing/font-size, or contextual-spacing-only cases.
...
- Structural empty body/header/footer paragraphs are now modeled for blank/terminal block cases; deeper count drift remains when the missing paragraph is tied to table/note/content-control nesting or non-terminal layout structure.
...
- Remaining layout/protocol gaps are richer floating/anchored positioning, embedded chart payload coverage beyond references/basic cached series, section-scoped element/layout mapping, and protocol-level full style inheritance.
- Protocol-level style inheritance is still partial: the current pass captures docDefaults spacing, direct run/paragraph properties, paragraph style definitions, renderer-side paragraph basedOn inheritance, actual table width for bbox, and common anchored image/chart bbox alignment, but it does not yet fully materialize paragraph style inheritance, false-value overrides, latent style defaults, character styles, or richer table border/span/style regions.
