...
- [ ] DOCX remaining advanced document features: bookmarks, equations, richer floating/anchored positioning, embedded chart payload coverage beyond references, complex section/layout handling, and full style inheritance.
- [ ] XLSX precise table styles: parse theme/table style metadata instead of hardcoding `TableStyleMedium2` preview colors.
- [ ] XLSX icon-set details: honor `cfvo` type/value, `percent`, `reverse`, and `showValue` rules.
...
- Remaining gaps are bookmarks, equations, richer floating/anchored positioning, embedded chart payload coverage beyond references, complex section layouts, and full style inheritance.
