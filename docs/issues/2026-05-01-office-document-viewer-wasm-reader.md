...
- [x] DOCX debug renderer now consumes decoded section header/footer content, paragraph alignment, margin/hanging indent, line spacing, list auto-number markers, run hyperlinks, DOCX highlight/caps scheme metadata, footnote/comment reference markers and bodies, insertion review marks, table bbox/widths/spans/margins/borders, image bbox sizing, and chart references.
...
- [ ] XLSX chart fidelity: preview consumes `sheet.drawings[].chart` anchors/series/legend, major chart families, combo hints, secondary axes, trendlines/error bars, data-label flags, axis titles, legend spacing, and formatted tick gutters; Excel/Walnut pixel-level chart typography and internal layout remain visual long-tail work.
...
- `npx vitest run src/app/debug/office-wasm-poc/__tests__/word-preview.test.tsx` passed after adding DOCX renderer coverage for section header/footer content, paragraph alignment/indent/line spacing/list markers, hyperlinks, DOCX highlight/caps scheme metadata, footnote/comment reference markers and bodies, insertion review marks, table bbox/spans/widths/margins/anchors/borders, image bbox sizing, and chart-reference canvas rendering.
