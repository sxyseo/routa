...
- [ ] DOCX remaining advanced document features: richer floating/anchored positioning, embedded chart payload coverage beyond references, complex section/layout handling, and full style inheritance.
- [ ] XLSX precise table styles: parse theme/table style metadata instead of hardcoding `TableStyleMedium2` preview colors.
- [x] XLSX icon-set threshold logic: honor `cfvo` type/value for min/max/num/percent/percentile, plus `gte`, `reverse`, and `showValue` rules.
- [ ] XLSX chart fidelity: improve plot area, legend, markers, gridlines, fonts, and Excel-like internal layout.
- [ ] XLSX freeze panes / sticky headers: reuse the explicit prefix-sum layout adapter for frozen viewport projection and hit regions.
- [ ] XLSX conditional formatting breadth: negative data bars, axes, and gradient/solid variants are implemented; richer color scales and layered rules still need more coverage.
...
- Remaining gaps are richer floating/anchored positioning, embedded chart payload coverage beyond references, complex section layouts, and full style inheritance.
