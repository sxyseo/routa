...
- Latest production-corpus decoded diff snapshot: `Copy of Appendix 1_2024 ePayments Reqs.xlsx`, `Copy of JC Penny  - Vendor Onboarding Questionnaire_.xlsx`, and `Copy of Thoughtworks foundational elements & sensible defaults.xlsx` are at `0` decoded diffs. Excluding the Org Chart image-payload case, remaining per-file decoded diffs range from `1` to `86`; the highest non-image files are `Thoughtworks Response DTA Cloud Marketplace Part B Attachment B - Pricing Schedule.xlsx` (`86`), `Proposal Workbench Tracker.xlsx` (`61`), and `Thoughtworks Response DTA Cloud Marketplace Part B Attachment A - Category Capability and Experience.xlsx` (`54`).
- The production-corpus top remaining paths are now font/style defaults (`styles.fonts[].fontSize`, font colors, typeface, bold/underline/scheme), rich text inherited bold in shared-string runs, XLSX drawing shape protocol (`shape.bbox`, `shape.paragraphs`, `shape.textStyle`, `shape.type`, shape line/connectors), and workbook image payload selection/deduplication for `Thoughtworks Org Chart.xlsx` (`Routa images=80`, `Walnut images=15`).

## Remaining XLSX Work

- Next decoded-protocol fixture expansion should cover broader theme/style combinations. Sheet-level `timelines` should be revisited with a real Excel-authored timeline fixture because the minimal OpenXML fixture only produces Walnut root `timelineCaches`. External-link parts were probed and Walnut did not expose them beyond formula-cell defaults.
...
- Freeze panes and sticky headers: the prefix-sum layout adapter now drives fixed headers, frozen body overlays, viewport projection, visible-grid windowing, cell hit regions, and floating drawing hit-region segmentation. Remaining work is extracting freeze panes from a future protocol source and wiring these regions into richer pointer interactions.
...
- Protocol coverage: `complex_excel_renderer_test.xlsx`, `xlsx_image_drawing_contract.xlsx`, `xlsx_sparkline_contract.xlsx`, `xlsx_defined_names_contract.xlsx`, `xlsx_comments_contract.xlsx`, `xlsx_threaded_comments_contract.xlsx`, `xlsx_pivot_contract.xlsx`, `xlsx_slicer_contract.xlsx`, `xlsx_timeline_contract.xlsx`, `xlsx_multi_chart_contract.xlsx`, `xlsx_surface_chart_contract.xlsx`, and `xlsx_external_link_contract.xlsx` now pass decoded Walnut Workbook protocol diff with zero field-level differences. Remaining work is raw protobuf byte exactness, understanding whether byte drift matters for downstream consumers, and adding more XLSX fixtures for sheet-level timelines and richer theme/style combinations.
- Production XLSX corpus: current contract gap is no longer basic parsing or table/chart core protocol. Remaining production-driven work is to add safe, non-production fixtures that cover font/style default ordering, shared-string inherited run styles, worksheet shape text/bbox/connectors, and image payload deduplication/selection. `/Users/phodal/Downloads/excel` is a validation corpus only and must not be committed.
