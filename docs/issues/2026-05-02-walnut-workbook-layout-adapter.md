...
- Next XLSX protocol pass should target workbook surfaces that still lack dedicated decoded fixtures: external links, richer chart families, and broader theme/style combinations. Sheet-level `timelines` should be revisited with a real Excel-authored timeline fixture because the minimal OpenXML fixture only produces Walnut root `timelineCaches`.
