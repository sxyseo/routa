...
- Production XLSX corpus: current contract gap is no longer basic parsing, table/chart core protocol, wide-row extraction, workbook image payload selection, direct shape `xfrm` bbox/type extraction, or basic worksheet shape text/body-style extraction. Remaining production-driven work is to add safe, non-production fixtures that cover font/style default ordering, shared-string inherited run styles, worksheet shape paragraph spacing/bullets/run splitting, connector defaults, and no-fill line defaults. `/Users/phodal/Downloads/excel` is a validation corpus only and must not be committed.
