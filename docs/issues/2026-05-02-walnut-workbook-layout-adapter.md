...
- Conditional formatting breadth: color scales, data bars, icon sets, common text/cell rules, duplicate/unique rules, negative data bars, explicit axes, cfvo thresholds, and `stopIfTrue` precedence for matched format rules are handled. Remaining work is wider precedence coverage across less-common Excel conditional-format rule families such as top/bottom, above/below average, and formula-driven custom rules.
