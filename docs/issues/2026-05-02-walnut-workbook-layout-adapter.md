...
- Production XLSX corpus: current contract gap is no longer basic parsing, table/chart core protocol, wide-row extraction, workbook image payload selection, or direct shape `xfrm` bbox/type extraction. Remaining production-driven work is to add safe, non-production fixtures that cover font/style default ordering, shared-string inherited run styles, and worksheet shape text/connectors/line defaults. `/Users/phodal/Downloads/excel` is a validation corpus only and must not be committed.
