...
- Conditional formatting breadth: color scales, data bars, icon sets, text/cell rules, negative data bars, explicit axes, cfvo thresholds, and `stopIfTrue` precedence for matched format rules are handled. Remaining work is wider precedence coverage across less-common Excel conditional-format rule families.
