...
- Production XLSX corpus: current contract gap is no longer basic parsing, table/chart core protocol, wide-row extraction, or workbook image payload selection. Remaining production-driven work is to add safe, non-production fixtures that cover font/style default ordering, shared-string inherited run styles, and worksheet shape text/bbox/connectors. `/Users/phodal/Downloads/excel` is a validation corpus only and must not be committed.
