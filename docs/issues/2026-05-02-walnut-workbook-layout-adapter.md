...
- Protocol coverage: `complex_excel_renderer_test.xlsx`, `xlsx_image_drawing_contract.xlsx`, `xlsx_sparkline_contract.xlsx`, `xlsx_defined_names_contract.xlsx`, `xlsx_comments_contract.xlsx`, `xlsx_threaded_comments_contract.xlsx`, `xlsx_pivot_contract.xlsx`, `xlsx_slicer_contract.xlsx`, `xlsx_timeline_contract.xlsx`, `xlsx_multi_chart_contract.xlsx`, `xlsx_surface_chart_contract.xlsx`, and `xlsx_external_link_contract.xlsx` now pass decoded Walnut Workbook protocol diff with zero field-level differences. The validation-only production corpus in `/Users/phodal/Downloads/excel` also passes decoded Walnut Workbook protocol diff with zero field-level differences across all 21 files. Remaining work is raw protobuf byte exactness, understanding whether byte drift matters for downstream consumers, and adding safe synthetic fixtures for sheet-level timelines and richer theme/style combinations.
- Production XLSX corpus: current contract gap is no longer basic parsing, table/chart core protocol, wide-row extraction, workbook image payload selection, direct shape `xfrm` bbox/type extraction, worksheet shape text/body-style extraction, stylesheet font ordering/defaults, shared-string inherited run styles, shape paragraph breaks/defaults, connector defaults, color tint transforms, indexed font colors, or no-fill shape defaults. `/Users/phodal/Downloads/excel` remains a validation corpus only and must not be committed.
