...
- Freeze panes and sticky headers: the prefix-sum layout adapter now drives fixed headers, frozen body overlays, viewport projection, visible-grid windowing, and cell hit regions. Remaining work is extracting freeze panes from a future protocol source and extending the same hit-region model to floating drawings/charts.
